...
export default function DemotionsPage() {
  const { data: lists, loading, refetch } = useFetch<RankChangeList[]>('/api/rank-change-lists?type=DEMOTION')
  ...
  return (
    <div>
                ...
                <div className="px-5 pb-4">
                  ...
                  {isDraft && (
                    <div className="flex gap-1.5">
                      ...
                      {pendingCount > 0 && (
                        <Button variant="danger" size="sm" onClick={() => setExecuteListId(list.id)}>
      ...
      <Modal open={!!executeListId} onClose={() => setExecuteListId(null)} title="Degradierungen ausführen">
        <p className="text-[13px] text-[#888] mb-5">
          Alle offenen Degradierungen in dieser Liste werden jetzt durchgeführt. Die Ränge der Officers werden sofort geändert. Fortfahren?
        </p>
        <div className="flex justify-end gap-2">
          <Button variant="secondary" size="sm" onClick={() => setExecuteListId(null)}>Abbrechen</Button>
          <Button variant="danger" size="sm" onClick={handleExecuteList}>Alle ausführen</Button>
        </div>
      </Modal>
    </div>
  )

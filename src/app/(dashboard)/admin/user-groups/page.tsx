    ...
    <div>
      ...
      <div className="glass-panel-elevated rounded-[12px] overflow-hidden">
        <div className="divide-y divide-line">
          {groups?.map((group, i) => (
            ...
            >
              <div className="h-8 w-8 rounded-full bg-surface-2 flex items-center justify-center text-[12px] font-semibold text-gold">
                {group.name.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13.5px] font-medium text-label">{group.name}</p>
                <p className="text-[11.5px] text-label-4 truncate">
                  {group.description || 'Keine Beschreibung'} · {group.permissions.length} Rechte · {group._count.users} Benutzer
                </p>
              </div>
              <div className="flex gap-0.5">
                <button onClick={() => openEdit(group)} className="p-1.5 rounded-[6px] hover:bg-surface-2 transition-colors">
                  <Edit size={13} className="text-label-4" />
                </button>
                <button onClick={() => deleteGroup(group)} className="p-1.5 rounded-[6px] hover:bg-red/14 transition-colors">
          ...
          {(!groups || groups.length === 0) && (
            <div className="text-center py-16">
              <Users size={28} className="mx-auto mb-3 text-label" strokeWidth={1.5} />
              <p className="text-[13px] text-label-3">Keine Benutzergruppen vorhanden</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editGroup ? 'Benutzergruppe bearbeiten' : 'Neue Benutzergruppe'} size="lg">
        <div className="space-y-4">
          ...
          <div>
            <p className="block text-[12.5px] font-medium text-label-2 mb-2">Leserechte</p>
          ...
          <div>
            <p className="block text-[12.5px] font-medium text-label-2 mb-2">Verwaltungsrechte</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">

...
function DatabaseCard({
  ...
  return (
    <div
      className={cn(
        'glass-panel-elevated rounded-[14px] p-4 border',
        active ? 'border-[#d4af37]/40' : 'border-white/5',
      )}
    >
      <div className="flex items-start gap-3">
        <div
          className={cn(
            ...
            active
              ? 'bg-gradient-to-br from-[#d4af37]/20 to-[#d4af37]/5 border-[#d4af37]/25 text-[#d4af37]'
              : 'bg-[#0a1e38]/60 border-white/5 text-[#8ea4bd]',
          )}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="text-[13.5px] font-semibold text-white truncate">{title}</p>
            {active && (
              <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#d4af37] border border-[#d4af37]/30 rounded-full px-2 py-0.5">
                aktiv
              </span>
            )}
          </div>
          <p className="text-[12px] text-[#8ea4bd] mt-0.5">{subtitle}</p>
...
export default function FailoverPage() {
  ...
  if (!data?.configured) {
    return (
      <div>
        ...
        />
        <div className="glass-panel-elevated rounded-[14px] p-5 border border-amber-500/20">
          <div className="flex items-start gap-3">
            <AlertTriangle className="h-4 w-4 text-amber-400 mt-0.5 shrink-0" />
            <div>
              <p className="text-[13.5px] font-semibold text-white">Noch nicht eingerichtet</p>
              <p className="text-[12.5px] text-[#8ea4bd] mt-1 leading-relaxed">
                Setze <code className="text-[#d4af37]">DATABASE_URL_STANDBY</code> in der <code>.env</code> auf die
  ...
  return (
    <div>
      ...
      {standbyActive && (
        <div className="rounded-[10px] bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/25 px-4 py-3 mb-5 flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[12px] text-[#b9c8da] mt-0.5 leading-relaxed">
      ...
      <div className="glass-panel-elevated rounded-[14px] p-5 mb-5">
        <p className="text-[13.5px] font-semibold text-white mb-1">Umschalten</p>
        <p className="text-[12px] text-[#8ea4bd] mb-4 leading-relaxed">
      ...
      {standbyActive && (
        <div className="glass-panel-elevated rounded-[14px] p-5 mb-5">
          <p className="text-[13.5px] font-semibold text-white mb-1">Schreib-Journal</p>
          <p className="text-[12px] text-[#8ea4bd] mb-4 leading-relaxed">
          ...
          {data.journal.failed.length === 0 ? (
            <p className="text-[12px] text-[#8ea4bd]">Keine fehlgeschlagenen Einträge.</p>
          ) : (
            <div className="space-y-2">
              {data.journal.failed.map((entry) => (
                <div
                  key={entry.seq}
                  className="rounded-[8px] border border-red-500/20 bg-[#1c1111]/40 px-3 py-2.5 flex items-start gap-3"
                >
                  <div className="min-w-0 flex-1">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-[6px] hover:bg-[#0f2340] transition-colors text-[#8ea4bd] shrink-0"
                    title="Überspringen"
                  >
      ...
      <div className="glass-panel-elevated rounded-[14px] p-5">
        <div className="flex items-start justify-between gap-4 mb-1">
          <p className="text-[13.5px] font-semibold text-white">Abgleich über die Backup-Dateien</p>
          <Button
            variant="secondary"
        ...
        {data.lastSync ? (
          <div className="grid gap-2 sm:grid-cols-3">
            <div>
              <p className="text-[11px] uppercase tracking-[0.1em] text-[#8ea4bd]">Zuletzt</p>
              <p className="text-[13px] text-white mt-0.5">{dateLabel(data.lastSync.finishedAt)}</p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.1em] text-[#8ea4bd]">Datenstand</p>
              <p className="text-[13px] text-white mt-0.5">{dateLabel(data.lastSync.snapshotAt)}</p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.1em] text-[#8ea4bd]">Zeilen</p>
              <p className="text-[13px] text-white mt-0.5">{data.lastSync.rows.toLocaleString('de-DE')}</p>
            </div>
            {data.lastSync.error && (
          ...
          <p className="text-[12px] text-[#8ea4bd]">Noch kein Abgleich gelaufen.</p>
        )}
      </div>
    </div>
  )

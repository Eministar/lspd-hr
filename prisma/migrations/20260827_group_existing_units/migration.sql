-- Bestehende Units werden ausschließlich gruppiert. Es werden keine Units,
-- Officer-Zuweisungen oder direkten Benutzerzuweisungen gelöscht.
-- INSERT IGNORE macht die Migration bei einem erneuten Deploy idempotent.
...
-- HR: Hauptunit, Navigation-Unit und sämtliche vorhandenen Ränge.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'HUMAN_RESOURCES'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('HUMAN_RESOURCES', 'HR', 'HR_LEITUNG', 'HR_STV_LEITUNG', 'HR_SUPERVISOR', 'SENIOR_HR_OPERATOR', 'HR_OFFICER', 'HR_TRAINEE');
...
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('ACADEMY', 'ACADEMY_LEITUNG', 'ACADEMY_STV_LEITUNG', 'RECRUITMENT_TRAINING_SENIOR_AUSBILDER', 'ACADEMY_AUSBILDER', 'RECRUITMENT_TRAINING_MENTOR', 'ACADEMY_TEST_AUSBILDER');

-- SRU inklusive der bisherigen Schreibweisen und Team-Unterunits.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'SRU'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('SRU', 'S_R_U', 'S_R_U_LEITUNG', 'S_R_U_STV_LEITUNG', 'S_R_U_SENIOR_OPERATOR', 'S_R_U_OPERATOR', 'S_R_U_TRAINEE', 'S_R_U_TEAM_ALPHA', 'METRO_TEAM_BETA', 'METRO_TEAM_GAMMA');

-- Air Support inklusive Alt- und Test-Units.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'AIR_SUPPORT'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('AIR_SUPPORT', 'AIR_SUPPORT_DIVISION', 'AIR_SUPPORT_DIVISION_LEITUNG', 'AIR_SUPPORT_DIVISION_MEMBER', 'TEST_AIR_SUPPORT_DIVISION');

-- Detective Unit inklusive der alten Basis-Unit.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'DETECTIVE'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('DETECTIVE', 'DETECTIVE_UNIT', 'DETECTIVE_LEITUNG', 'DETECTIVE_STV_LEITUNG', 'DETECTIVE_OFFICER', 'DETECTIVE_TRAINEE');

-- Pressestellen, Social Media und Streamer.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'PRESS'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('PRESS', 'PRESSESPRECHER', 'STREAMER', 'SOCIAL_MEDIA');

-- Internal Affairs inklusive Agenten-Ränge.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'INTERNAL_AFFAIRS'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` IN ('INTERNAL_AFFAIRS', 'INTERNAL_AFFAIRS_JUNIOR_AGENT', 'INTERNAL_AFFAIRS_OPERATOR', 'INTERNAL_AFFAIRS_LEITUNG', 'INTERNAL_AFFAIRS_STV_LEITUNG');

-- Die alleinstehende Rechtsvertretungs-Unit bleibt erhalten, wird aber als
-- eigene Gruppe geführt, damit sie später ohne Datenumbau erweitert werden kann.
UPDATE `Unit` u
JOIN `UnitGroup` g ON g.`key` = 'RECHTSVERTRETUNG'
SET u.`groupId` = g.`id`
WHERE u.`groupId` IS NULL
  AND u.`key` = 'RECHTSVERTRETUNG';

-- Leitungseinheiten erhalten automatisch die gemeinsame Leitungsrolle ihrer
-- Gruppe. Bestehende manuelle Markierungen werden nicht zurückgesetzt.
UPDATE `Unit`
